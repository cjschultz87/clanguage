...
int btd(array alpha, int start, int length)
{
	int rVal = 0;

	for (int i = 0; i < length; i++)
	{
		rVal += alpha.loc[start + i] * power(2,length - (i + 1));
	}
...
void main(int argc, char **argv)
{
	...
	else
	{
		...
		if (strCompare(strSubI(strLength(path_in)-3,3,path_in),".fe") == 1)
		{
			...
			str blank = strToN(strLength(path_in),10);
	...
	errno_t echo_0, echo_1;
	
	echo_0 = fopen_s(&foxtrot_0, path_in, "rb");
	...
	echo_1 = fopen_s(&foxtrot_1, path_out, "wb");
	...
	for (int i = 0; i < fSize0; i++)
	{
		...
		unsigned char fox_byte = getc(foxtrot_0);
	...
	if (fEncryption == 0)
	{
		for (int i = 0; i < fK_1.Length; i += 8)
		{
			...
			{	
				unsigned char sierra_byte = 0;
				
				sierra_byte = btd(fSIERRA, i_1 * 8, 8);
				
				sierra_byte = sierra_byte ^ fK_1.loc[i];
				
				sierra_byte = sierra_byte ^ carry;
	...
	if (fEncryption == 1)
	{
		...
		{	
			...
			for (int i_1 = fSize0 - 2; i_1 >= 0; i_1 -= 2)
			{	
				unsigned char sierra_byte = 0;
				...
				for (int i_2 = 0; i_2 < fS_0.Length; i_2++)
				{
					fSIERRA.loc[(i_1*8)+i_2] = fS_0.loc[7-i_2];
				}
			}
